...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        # print minFoodDistance , minGhostDistance, successorGameState.getScore()
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def minValue(self, gameState, agent, depth):
        ...
        listActions = [gameState.generateSuccessor(agent, action) for action in ghostActions]

        if gameState.isWin() or gameState.isLose():
            return self.evaluationFunction(gameState)

        if agent == gameState.getNumAgents() - 1:
            return min([self.maxValue(successor, depth + 1) for successor in listActions])
        else:
            return min([self.minValue(successor, agent + 1, depth) for successor in listActions])
    ...
    def minValue(self, gameState, agent, depth, alpha, beta):
        ...
        for action in ghostActions:
            ...
            if agent == gameState.getNumAgents() - 1:
                ...
                cost = self.minValue(successor, agent + 1, depth, alpha, beta)[0]
